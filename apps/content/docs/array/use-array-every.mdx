...
`useArrayEvery` is a React hook that provides a memoized implementation of the Array.prototype.every method.
...
<TypeTable
  type={{
    boolean: {
      description:
        'true if the predicate function returns a truthy value for every element from the array. Otherwise, false.',
    },
  }}
/>
