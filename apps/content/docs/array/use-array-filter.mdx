...
`useArrayFilter` is a React hook that provides a memoized implementation of the `Array.prototype.filter` method.

<TypeTable
  type={{
    list: {
      description:
        'The array to filter. If list is not an array, an error message is logged to the console and the hook returns an empty array.',
      type: 'T[]',
    },
    predicate: {
      description:
        'A function that is called for every element of the given list, returning true for elements to include in the new array.',
      type: 'UseArrayFilterPredicate',
    },
  }}
/>
