...
import { type ProjectorFunction } from '../../math';
...
describe('useProjection', () => {
  ...
  it('should handle fromDomain as a function returning an array', () => {
  ...
  it('should handle toDomain as a function returning an array', () => {
    const toDomainFn = () => [0, 100] as const;
    const { result } = renderHook(() => useProjection(5, [0, 10], toDomainFn));
    expect(result.current).toBe(50);
  });

  it('should handle both fromDomain and toDomain as functions returning arrays', () => {
    const fromDomainFn = () => [0, 10] as const;
    const toDomainFn = () => [0, 100] as const;
    const { result } = renderHook(() =>
  ...
  it('should use the default projector when no custom projector is provided', () => {
    const { result } = renderHook(() => useProjection(5, [0, 10], [0, 100]));
    expect(result.current).toBe(50);
  });

  it('should project input value using a custom projector', () => {
    const { result } = renderHook(() =>
      useProjection(5, [0, 10], [0, 100], customProjector),
    );
    expect(result.current).toBe(50);
