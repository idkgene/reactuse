...
Te `useArrayUnique` hook provides a memoized function to extract unique elements from an array. It allows for custom comparison logic, making it versatile for various data structures and comparison needs.
...
`items`: The array of items to filter for unique values.

`compareFn`: (optional): A custom function to compare two elements. It receives three arguments:

- `a`: The first element to compare.
- `b`: The second element to compare.
- `array`: The entire array being processed.

If `compareFn` returns `true`, the elements are considered equal, and only one is kept. If omitted, a default comparison based on JSON stringification is used.

### Returns

### Returns

`T[]`: A new array containing only the unique elements from the input `items` array, determined using the provided `compareFn` or the default comparison.
