...
| ------------- | ----- | ------- | ----------------------------------------------------------------- |
| `defaultList` | `T[]` | -       | The initial array of elements. Defaults to an empty array (`[]`). |

### Returns

| Property  | Type                                                                                                                                                                                                         | Default | Description                                                       |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- | ----------------------------------------------------------------- |
| `T[]`     | `T[]`                                                                                                                                                                                                        | -       | The current state of the list as an array (`T[]`).                |
| `actions` | `{ set: (l: T[]) => void; push: (element: T) => void; removeAt: (index: number) => void; insertAt: (index: number, element: T) => void; updateAt: (index: number, element: T) => void; clear: () => void; }` | -       | An object containing the utility functions for managing the list. |
